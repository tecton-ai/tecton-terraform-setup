...
output "satellite_vpc_id" {
  value = var.satellite_regions == "" ? "" : module.satellite_subnets[0].vpc_id
}

output "satellite_eks_subnet_ids" {
  value = var.satellite_regions == "" ? [""] : module.satellite_subnets[0].eks_subnet_ids
}

output "satellite_public_subnet_ids" {
  value = var.satellite_regions == "" ? [""] : module.satellite_subnets[0].public_subnet_ids
}
...
output "satellite_security_group_ids" {
  value = [
    var.satellite_regions == "" ? "" : module.satellite_security_groups[0].eks_security_group_id, 
    var.satellite_regions == "" ? "" : module.satellite_security_groups[0].eks_worker_security_group_id,
    var.satellite_regions == "" ? "" : module.satellite_security_groups[0].rds_security_group_id
  ]
}
...
output "roles" {
  value = {
    ...
    fargate_satellite_kinesis_firehose_stream_role_name   = (var.apply_layer > 1) ? module.roles[0].fargate_satellite_kinesis_firehose_stream_role_name : ""
    fargate_satellite_eks_fargate_pod_execution_role_name = (var.apply_layer > 1) ? module.roles[0].fargate_satellite_eks_fargate_pod_execution_role_name : ""
  }
}
