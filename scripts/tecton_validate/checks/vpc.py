...
def _check_subnet_validity(args: argparse.Namespace, session: boto3.Session, console: Console) -> ValidationResult:
    ...
    if isinstance(subnet_ids_raw, str):
        ...
        subnet_ids = subnet_ids_raw if isinstance(subnet_ids_raw, list) else [str(subnet_ids_raw)]
    ...
    try:
        ...
        for subnet_id in subnet_ids:
            if subnet_id not in found_subnets:
                ...
                unavailable_subnets.append(f"{subnet_id} ({found_subnets[subnet_id]['State']})")
...
def _check_security_group_egress(args: argparse.Namespace, session: boto3.Session, console: Console) -> ValidationResult:
    ...
    try:
        ...
        for rule in egress_rules:
            ...
            for ip_range in ip_ranges:
                ...
                if cidr == '0.0.0.0/0' and ip_protocol == '-1':  # Allow all traffic to anywhere
            ...
            for ipv6_range in ipv6_ranges:
                ...
                if cidr == '::/0' and ip_protocol == '-1':  # Allow all traffic to anywhere (IPv6)
                    has_broad_egress = True
                egress_details.append(f"{ip_protocol}:{from_port}-{to_port} -> {cidr}")
...
# Mark these checks as only applicable for rift compute
_check_subnet_validity.only_for = ["rift"]  # type: ignore
_check_security_group_egress.only_for = ["rift"]  # type: ignore
