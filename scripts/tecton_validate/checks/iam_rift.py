...
from ..validation_types import ValidationCheck, ValidationResult
from ..policy_test import test_policy
from ..terraform import load_terraform_outputs
...
def _check_rift_role_existence(args: argparse.Namespace, session: boto3.Session, console: Console) -> ValidationResult:
    ...
    for rn in required:
        try:
            ...
            missing.append(rn)
    if missing:
        return ValidationResult("Rift compute IAM role existence", False, f"Missing roles: {', '.join(missing)}", "Run 'terraform apply' on rift_compute module")
    if empty:
        return ValidationResult("Rift compute IAM role existence", False, f"Roles without policies: {', '.join(empty)}", "Attach managed policies defined in templates/ directory")
    return ValidationResult("Rift compute IAM role existence", True, "All roles present with policies")


def _check_rift_policies(args: argparse.Namespace, session: boto3.Session, console: Console) -> ValidationResult:
    ...
    outputs = load_terraform_outputs(args.terraform_outputs, console) if args.terraform_outputs else {}
    ...
    allow_run = [f"arn:aws:ec2:*:{args.account_id}:volume/*", f"arn:aws:ec2:*:{args.account_id}:security-group/{sg_id}"] + [f"arn:aws:ec2:*:{args.account_id}:subnet/{s}" for s in subnet_ids or ["subnet-12345678"]]
    allow_iface = [f"arn:aws:ec2:*:{args.account_id}:security-group/{sg_id}"] + [f"arn:aws:ec2:*:{args.account_id}:subnet/{s}" for s in subnet_ids or ["subnet-12345678"]]
    ...
    return ValidationResult(
        "Rift compute IAM policy simulation" if success else "Rift compute IAM policy simulation",
        success,
        " | ".join(details),
...
CHECKS: List[ValidationCheck] = [
    ValidationCheck("Rift compute IAM role existence", _check_rift_role_existence, "Create the roles with rift_compute Terraform module."),
    ValidationCheck("Rift compute IAM policy simulation", _check_rift_policies, "Attach/verify policies for rift compute roles."),
] 