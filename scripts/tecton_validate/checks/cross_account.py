...
def _validate_cross_account(args: argparse.Namespace, session: boto3.Session, console: Console) -> ValidationResult:
    iam_client = session.client("iam")
    iam = session.resource("iam")

    tpl_dir = os.path.realpath(os.path.join(os.path.dirname(__file__), "../../../templates"))
    success = True
    ...
    try:
        ...
        ca_tpl = "ca_policy.json" if args.compute_engine in {"emr", "databricks"} else "rift_ca_policy.json"
        ...
        return ValidationResult(
            ...
            success,
            "All Cross-Account Role IAM policy simulations allowed." if success else "One or more IAM simulations failed – see console for details.",
...
CHECKS: List[ValidationCheck] = [
    ValidationCheck("SaaS IAM policy simulation", _validate_cross_account, "Attach/update IAM policies from templates directory."),
] 