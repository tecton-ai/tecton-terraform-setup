...
def load_terraform_outputs(outputs_file_path: Union[str, Path], console: Console) -> dict:
    ...
    try:
        ...
        if "tecton" in raw and isinstance(raw["tecton"], dict) and "value" in raw["tecton"]:
        ...
        console.print(f"[yellow]Terraform outputs file not found: {path} – continuing with defaults[/yellow]")
        return {}
    except json.JSONDecodeError as exc:
        console.print(f"[yellow]Could not parse outputs file: {exc} – continuing with defaults[/yellow]")
        return {} 