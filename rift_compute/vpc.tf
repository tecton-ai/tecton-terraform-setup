locals {
  ...
  vpc_id = local.is_existing_vpc ? var.existing_vpc.vpc_id : aws_vpc.rift[0].id
  ...
  private_subnet_ids = local.is_existing_vpc ? var.existing_vpc.private_subnet_ids : values(aws_subnet.private)[*].id
  private_subnet_arns = local.is_existing_vpc ? [for subnet_id in var.existing_vpc.private_subnet_ids : format("arn:aws:ec2:%s:%s:subnet/%s", data.aws_region.current.name, data.aws_caller_identity.current.account_id, subnet_id)] : values(aws_subnet.private)[*].arn
...
data "aws_subnet" "existing_private" {
  count    = local.is_existing_vpc && length(var.existing_vpc.private_subnet_ids) > 0 ? length(var.existing_vpc.private_subnet_ids) : 0
  id       = var.existing_vpc.private_subnet_ids[count.index]
}
