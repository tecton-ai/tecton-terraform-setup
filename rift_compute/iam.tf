...
resource "aws_iam_policy" "additional_s3_read_access" {
  count = length(var.additional_s3_read_access_bucket_arns) > 0 ? 1 : 0
  ...
  policy = templatefile("${path.module}/../templates/additional_s3_read_access_policy.json", {
    ADDITIONAL_S3_BUCKET_ARNS = jsonencode(var.additional_s3_read_access_bucket_arns),
    ADDITIONAL_S3_OBJECT_ARNS = jsonencode([for bucket_arn in var.additional_s3_read_access_bucket_arns : "${bucket_arn}/*"])
  })
}
...
locals {
  ...
  rift_compute_policies_with_legacy_secrets = merge(
  ...
  # Include additional S3 read access policy when buckets are specified
  rift_compute_policies = merge(
    local.rift_compute_policies_with_legacy_secrets,
    length(var.additional_s3_read_access_bucket_arns) > 0 ? {
